...
<?php foreach($posts as $k=>$v){ ?>
  ...
  <?php if($images){ foreach($images as $imgid=>$img){ if($img["postid"]!=$v["postid"]) continue; ?>
  <div class="image">
  ...
  <?php if (!isset($_SESSION['imgedit'])) {$_SESSION['imgedit']=False;}?>
  <?php if(!$_SESSION["imgedit"] or $_SESSION['postid']!=$v['postid']){ ?> 
  <input type="file" name="image" /> 
  <?php }else{ ?>
  Nowy opis ilustracji: 
  <?php } ?>
  <input type="text" name="imagetitle" value="<?=($_SESSION["imgedit"] and $_SESSION['postid']==$v['postid'])?$images[$_SESSION["imgid"]]["title"]:"";?>" placeholder="Opis pliku" /> <button type="submit" >Zapisz</button>
  <input type="hidden" name="postid" value="<?=$v['postid']?>" />
  </form><?php if(isset($uploaderror)){?><span class="error">ERROR!:<?=$uploaderror?></span><?php } ?>
